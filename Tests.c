...
void TestCreateLinearForm(){
    ...
    LinearFormErrors operation_result = LINEARFORM_OPERATION_OK;
    LinearForm* lf1 = CreateLinearForm(GetIntTypeInfo(), 10, &operation_result);
    if (operation_result != LINEARFORM_OPERATION_OK) result_of_test = 1;
...
void TestInputLinearForm(){
    ...
    if (operation_result != LINEARFORM_NOT_DEFINED) result_of_test = 1;
    TestResultPrint(&result_of_test, &test_group_number, &test_number);
    test_number++;
    freeLinearForm(lf3);
}
...
void TestAddLinearForm(){
    ...
    if (operation_result != INCOMPATIBLE_LINEARFORM_TYPES) result_of_test = 1;
    TestResultPrint(&result_of_test, &test_group_number, &test_number);
    test_number++;
...
void TestCountLinearForm(){
    ...
    if (operation_result != LINEARFORM_OPERATION_OK) {result_of_test = 1; printf("%d\n", operation_result);}
